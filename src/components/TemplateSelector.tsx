        ...
        {APP_TEMPLATE_OPTIONS.map((template) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="text-lg font-semibold text-white mb-2">{templateData.label}</h3>
              <p className="text-sm text-gray-400 mb-3">{templateData.description}</p>
              <div className="flex flex-wrap gap-2">
                {templateData.features.map((feature, index) => (
                  <span key={index} className="text-xs bg-gray-700 px-2 py-1 rounded-full text-gray-300">
                    {feature}
                  </span>
                ))}
              </div>
