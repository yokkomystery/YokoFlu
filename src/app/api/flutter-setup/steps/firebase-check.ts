...
export async function runFirebaseEnvironmentCheck({
  ...
  // Firebase CLI の存在確認
  try {
    const versionResult = await execFileAsync('firebase', ['--version']);
    updateProgress(
      'firebase-check',
    ...
    updateProgress(
      'firebase-check',
      '❌ Firebase環境の準備ができていません',
      'Firebase環境の準備ができていません'
    );
    recordStepResult(
      'firebase-check',
      'error',
      'Firebase CLIがインストールされていません',
      {
  ...
  try {
    const projectsResult = await execFileAsync('firebase', ['projects:list']);
    updateProgress(
      'firebase-check',
      '✅ Firebaseアカウントにログイン済みです',
      'Firebaseアカウントにログイン済みです'
    );
    recordStepResult(
      'firebase-check',
      'success',
      'Firebase CLIにログイン済みです',
      {
        projectsCount:
    ...
    if (separateEnvironments) {
      ...
      if (!availableProjects.includes(stagingProjectId)) {
        console.warn(
          `⚠️ Stagingプロジェクト "${stagingProjectId}" が見つかりません`
        );
      }
      if (!availableProjects.includes(productionProjectId)) {
        console.warn(
          `⚠️ Productionプロジェクト "${productionProjectId}" が見つかりません`
        );
      }
    } else {
